...
for(workerID in c(0:idMax)) {
  ...
  if(length(idICases)>0) {
    ...
    optRes1 <- optimize(RSAModelUttKLDivParamD4A.2, c(-10,10), dataWorker)
    print(optRes1$objective)
    optRes2 <- optimize(RSAModelUttKLDivParamD4B.2, c(-10,10), dataWorker)   
    print(optRes2$objective)
    optRes3 <- optimize(RSAModelUttKLDivParamD4AB.2, c(-10,10), dataWorker)   
    ...
    ####
    optRes2n1 <- optim(c(.2, .2), RSAModelUttKLDivParamBD4A.2, method="L-BFGS-B", gr=NULL, dataWorker,
                       lower = c(0,-10), upper = c(1e+10,10))
    print(optRes2n1$value)
    optRes2n2 <- optim(c(.2, .2), RSAModelUttKLDivParamAD4B.2, method="L-BFGS-B", gr=NULL, dataWorker,
                       lower = c(0,-10), upper = c(1e+10,10))
...
write.csv(klDivUttWorkers, "KLDivUttWorkersWith.2_x3_2019_05_16.csv")
write.csv(paramsUttWorkers, "KLDivUttParamsWorkersWith.2_x3_2019_05_16.csv")

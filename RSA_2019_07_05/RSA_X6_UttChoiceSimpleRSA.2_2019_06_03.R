...
x6pilotData <- read.csv("6-combined-revised.csv")
...
workerIDs <- x6pilotData$workerid
idMax <- max(workerIDs)
...
for(workerID in c(0:idMax)) {
  ...
  if(length(idICases)>0) {
    ...
    for(i in c(1:length(idICases))) {
      len <- x6pilotData$numFeatures[idICases[i]]
      for(j in c(1:len) ) {
...
for(workerID in c(0:idMax)) {
  ...
  if(length(idICases)>0) {
    ...
    dataWorker[,4] <- x6pilotData$numFeatures[idICases]
...
#    print(optRes1$objective)
    ...
    optRes3 <- optimize(SimpleRSAModelUttKLDivParamK.2.2, c(-10,10), dataWorker)   
...
#    paramsUttWorkers[workerIndex,2] <- optRes1$minimum
    ...
    ####
    optRes2n1 <- optim(c(.2, .2), SimpleRSAModelUttKLDivParamBK.2, method="L-BFGS-B", gr=NULL, dataWorker,
                       lower = c(0,-10), upper = c(1e+10,10))
    print(optRes2n1$value)
    optRes2n2 <- optim(c(.2, .2), SimpleRSAModelUttKLDivParamAK.2, method="L-BFGS-B", gr=NULL, dataWorker,
                       lower = c(0,-10), upper = c(1e+10,10))
     print(optRes2n2$value)
#    optRes2n3 <- optim(c(.2, .2), SimpleRSAModelUttKLDivParamAB(), method="L-BFGS-B", gr=NULL, dataWorker,
...
write.csv(klDivUttWorkers, "KLDivUttWorkers_x6_UttChoiceSimpleRSA.2_2019_06_03.csv")
write.csv(paramsUttWorkers, "KLDivUttParamsWorkers_x6_UttChoiceSimpleRSA.2_2019_06_03.csv")

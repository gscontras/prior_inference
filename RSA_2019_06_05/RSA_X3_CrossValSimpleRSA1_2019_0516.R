...
source("RSA_UttChoiceOptimization_SimpleRSA_2019_0516.R")
...
for(workerID in c(0:idMax)) {
  ...
  if(length(idICases)>0) {
    ...
    for(workerTrialIndex in c(1:length(idICases))) {
      ...
      if(parOptType==1) {
        optRes1 <- optimize(SimpleRSAModelUttKLDivParamB, c(0,1e+10), dataWorker)
        ...
        optRes2 <- optim(c(.2, 0), SimpleRSAModelUttKLDivParamBK, method="L-BFGS-B", gr=NULL, dataWorker,
        ...
        optRes3 <-  optim(c(.2, .2), SimpleRSAModelUttKLDivParamAB, method="L-BFGS-B", gr=NULL, dataWorker,
      ...
      if(parOptType==1) {
        klDivCrossValValues[workerIndex, workerTrialIndex] <-
          SimpleRSAModelUttKLDiv_3params(dataTrialWorker, 0.2, optRes1$minimum, 1)
      }else if(parOptType==2) {
        klDivCrossValValues[workerIndex, workerTrialIndex] <- 
          SimpleRSAModelUttKLDiv_3params(dataTrialWorker, 0.2, optRes2$par[1], optRes2$par[2])
      }else if(parOptType==3) {
        klDivCrossValValues[workerIndex, workerTrialIndex] <-
          SimpleRSAModelUttKLDiv_3params(dataTrialWorker, optRes3$par[1], 0.2, optRes3$par[2])
      }else if(parOptType==4) {
        klDivCrossValValues[workerIndex, workerTrialIndex] <-
...
write.csv(klDivUttWorkers, "KLDiv_UttChoice_CrossVal_Workers.csv")
write.csv(paramsUttWorkers, "KLDiv_UttChoice_CrossVal_ParamsWorkers.csv")

## 
## writing out tables
if(parOptType==1) {
  write.csv(klDivUttWorkers, "x3CrossVal_SimpleRSA_KLDivOpt_ObedPar_2019_0516.csv")
  write.csv(paramsUttWorkers, "x3CrossVal_SimpleRSA_Params_ObedPar_2019_0516.csv")
  write.csv(klDivCrossValValues, "x3CrossVal_SimpleRSA_KLDivTrial_ObedPar_2019_0516.csv")
}else if(parOptType==2) {
  write.csv(klDivUttWorkers, "x3CrossVal_SimpleRSA_KLDivOpt_ObedAndKPars_2019_0516.csv")
  write.csv(paramsUttWorkers, "x3CrossVal_SimpleRSA_Params_ObedAndKPars_2019_0516.csv")
  write.csv(klDivCrossValValues, "x3CrossVal_SimpleRSA_KLDivTrial_ObedAndKPars_2019_0516.csv")
}else if(parOptType==3) {
  write.csv(klDivUttWorkers, "x3CrossVal_SimpleRSA_KLDivOpt_PrefAndKPars_2019_0516.csv")
  write.csv(paramsUttWorkers, "x3CrossVal_SimpleRSA_Params_PrefAndKPars_2019_0516.csv")
  write.csv(klDivCrossValValues, "x3CrossVal_SimpleRSA_KLDivTrial_PrefAndKPars_2019_0516.csv")
}else if(parOptType==4) {
  write.csv(klDivUttWorkers, "x3CrossVal_SimpleRSA_KLDivOpt_PrefObedKPars_2019_0516.csv")
  write.csv(paramsUttWorkers, "x3CrossVal_SimpleRSA_Params_PrefObedKPars_2019_0516.csv")
  write.csv(klDivCrossValValues, "x3CrossVal_SimpleRSA_KLDivTrial_PrefObedKPars_2019_0516.csv")
}

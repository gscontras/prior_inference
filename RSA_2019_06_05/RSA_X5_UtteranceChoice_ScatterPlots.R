...
x5pilotData <- read.csv("x5pilotDataAugm_UttChoice_fixedAndD_2019_05_29.csv")
x5pilotData <- x5pilotData[which(x5pilotData$trial_type=="utterance_choice"),]

## adding the 1-27 target and object2 & object3 code.
o1 <- x5pilotData$obj1OC27
o2 <- x5pilotData$obj2OC27
o3 <- x5pilotData$obj3OC27
##

########### 
## filtering for only the present feature values for each feature.
###########
subjectGuessIndexM1 <- grep("^DPost_1", colnames(x5pilotData)) - 1
modelGuessIndex1M1 <- grep("^MPost1_1", colnames(x5pilotData)) - 1
modelGuessIndex2M1 <- grep("^MPost2_1", colnames(x5pilotData)) - 1
  ...
  for(j in c(1:3)) { # iterating over the three feature types
    ...
    for(v in c(1:3)) {
      if(length(which(valUttRel == ((j-1)*3) + v )) == 0) {
        ...
        x5pilotData[[modelGuessIndex1M1 + ((j-1)*3) + v]][i] <- NA
        x5pilotData[[modelGuessIndex2M1 + ((j-1)*3) + v]][i] <- NA
      }
    }
  }
}
...
subjectGuessIndex <- grep("^DPost_1", colnames(x5pilotData))
modelGuessIndex1 <- grep("^MPost1_1", colnames(x5pilotData))
modelGuessIndex2 <- grep("^MPost2_1", colnames(x5pilotData))

x5pilotData$CCode <- uniqueCCode
x5pilotData$featValOrder <- featureValueOrder

modelDataOrdered <- matrix(-1,nrow(x5pilotData),16)
for(i in c(1:length(x5pilotData$X))) {
  for(j in c(1:length(x5pilotData$featValOrder[[i]]))) {
      modelDataOrdered[i,j] <- x5pilotData[[modelGuessIndex2M1+x5pilotData$featValOrder[[i]][j]]][i] 
  }
}
modelDataOrdered[,10] <- uniqueCCode
modelDataOrdered[,11] <- x5pilotData$obj1
modelDataOrdered[,12] <- x5pilotData$obj2
modelDataOrdered[,13] <- x5pilotData$obj3
modelDataOrdered[,14] <- x5pilotData$obj1OC27
modelDataOrdered[,15] <- x5pilotData$obj2OC27
modelDataOrdered[,16] <- x5pilotData$obj3OC27
write.csv(modelDataOrdered, "x5pilotDataModelOptimizedSorted.csv")
################################################################################

...
simpleBestInfGainUtterance <- function(preferencesPrior, relevantUtterances, currentObjects, 
  ...
  for(utt in c(1:length(relevantUtterances))) { # evaluating the expected utility of uttering a particular utterance utt
    ...
    for(listenerPref in c(1:length(preferencesPrior))) { # prior over the preferences the speaker is interested in
      ...
      for(objChoice in c(1:length(currentObjects)) ) {
        if(mapUttToObjProbs[utt,objChoice] > 0) {
          if(preferencesPrior[listenerPref] > 0) { # only pay attention to preferences with non-zero probability
            featurePrefsPosterior <- simplePragmaticSpeaker(utt, objChoice, preferencesPrior, 
            ...
            probabilityOfSituation <- mapUttToObjProbs[utt,objChoice] * utterancePrior[utt] * 
              preferencesPrior[listenerPref] * objectPreferenceSoftPriors[[listenerPref]][objChoice]
...
#                     mapUttToObjProbs, objectPreferenceSoftPriors) # sanity check - definite prior, no inf. gain possible
round(simpleBestInfGainUtterance(c(0, 0, 0, 1/2, 1/2, 0, 0), relevantUtterances, currentObjects,

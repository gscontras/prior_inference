...
if(FALSE) {
  bInfGainUttModel <- matrix(NA, nrow(x5pilotData), 9)
  for(i in c(1:nrow(x5pilotData))) {
...
if(doReRunOptimization) {
  ...
  write.csv(klDivUttWorkers, "x5UttChoiceModelsKLDivs.csv")
  write.csv(paramsUttWorkers, "x5UttChoiceModelsParams.csv")
...
if(!doReRunOptimization) {
  ...
  klDivUttWorkers <- as.matrix(read.csv("x5UttChoiceModelsKLDivs.csv"))
  klDivUttWorkers <- klDivUttWorkers[ , 2:ncol(klDivUttWorkers)]
  paramsUttWorkers <- as.matrix(read.csv("x5UttChoiceModelsParams.csv"))
...
postListMatNotOpt <- matrix(0,length(x5pilotData$X),9)
...
x5pilotData <- data.frame(x5pilotData, as.data.frame(postListMatNotOpt)) 

write.csv(x5pilotData, "x5pilotDataUttChoiceAugmented012019.csv")

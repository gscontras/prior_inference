...
idMax <- max(workerIDs)
llWorkers12 <- matrix(0,length(unique(workerIDs)), 6)
paramsWorkers12 <- matrix(0,length(unique(workerIDs)), 7)
##########
...
optRes12 <- optim(c(.2, .2), RSAModelLL2_simpleRSA, method="L-BFGS-B", gr=NULL, dataWorker,
                   lower = c(0,0), upper = c(1e+10,1e+10))

# print(optRes)
## 2 param simple RSA model2
## KL values and parameter choice
llWorkers12[1,6] <- optRes12$value
## max likelihood parameter choice
paramsWorkers12[1,6] <- optRes12$par[1]
paramsWorkers12[1,7] <- optRes12$par[2]
##
print(llWorkers12[1,])
print(paramsWorkers12[1,])
...
write.csv(llWorkers12, "X2_Data/x2ModelsKLDivsGlobalSimpleRSA_2019_0514.csv")
write.csv(paramsWorkers12, "X2_Data/x2ModelsOptParamsGlobalSimpleRSA_2019_0514.csv")
...
# 
# write.csv(x2pilotData, "X2_Data/x2pilotDataAugmented_2019_04_30.csv")
# 
# 
# 
# 

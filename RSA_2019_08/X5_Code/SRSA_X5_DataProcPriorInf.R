...
for(workerID in c(0:idMax)) {
  ...
  if(length(idICases)>0) {
    ...
    optRes1no.2 <- optimize(RSAModelLL1_1simpleRSA_notObey.2, c(0,1e+10), dataWorker)   
    optRes2 <- optimize(RSAModelLL1_2simpleRSA, c(.01,100), dataWorker)   
    optRes1no.1 <- optimize(RSAModelLL1_1simpleRSA_notObey.1, c(0,1e+10), dataWorker)   
    optRes2p.2 <- optimize(RSAModelLL1_2simpleRSA_pref2, c(.01,100), dataWorker)   
    #print(optRes)
    llWorkers12[workerIndex,1] <- workerID
    paramsWorkers12[workerIndex,1] <- workerID
    ## 1 param RSA model
    llWorkers12[workerIndex,3] <- optRes1$objective
    llWorkers12[workerIndex,4] <- optRes1no.2$objective
    llWorkers12[workerIndex,5] <- optRes2$objective
    llWorkers12[workerIndex,6] <- optRes1no.1$objective
    llWorkers12[workerIndex,7] <- optRes2p.2$objective
    ## resulting parameter choice
    paramsWorkers12[workerIndex,2] <- optRes1$minimum
    paramsWorkers12[workerIndex,3] <- optRes1no.2$minimum
    paramsWorkers12[workerIndex,4] <- optRes2$minimum
    paramsWorkers12[workerIndex,5] <- optRes1no.1$minimum
    paramsWorkers12[workerIndex,6] <- optRes2p.2$minimum
    ####
...
# before optimization:     llWorkers12[workerIndex,7] <- RSAModelLL2(c(.2,.2), dataWorker)
    ...
    llWorkers12[workerIndex,8] <- optRes2$value
    ## max likelihood parameter choice
    paramsWorkers12[workerIndex,7] <- optRes2$par[1]
    paramsWorkers12[workerIndex,8] <- optRes2$par[2]
    ##
    print(llWorkers12[workerIndex,])
    print(paramsWorkers12[workerIndex,])
    workerIndex <- workerIndex + 1
...
write.csv(llWorkers12, "X5_Data/x5PriorInfModelsKLDivsSimpleRSA_2019_05_17.csv")
write.csv(paramsWorkers12, "X5_Data/x5PriorInfModelsOptParamsSimpleRSA_2019_05_17.csv")

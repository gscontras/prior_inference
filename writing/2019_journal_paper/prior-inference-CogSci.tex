...
We ran two main types of experiments online using a modified version of the original reference game experiment \cite{frankgoodman2012} and modeled the recorded data by enhancing the Rational Speech Act framework.
...
closely linked to the predictive mind perspective \cite{Friston:2015,Hohwy:2013,Clark:2014}. 
...
% TODO - can one contrast this to our modeling perspective?
...
Our model builds on the vanilla version of RSA presented above, modifying the listener's state prior $P(s)$ and enhancing the inference process towards social model inference, yielding a full pragmatic social inference RSA model (fPSIRSA). %In that sense, the model belongs to the family of models known as uncertain RSA \cite{goodmanfrank2016}. 
...
Moreover, we can use the minimized KL divergence values to perform the likelihood ratio test for nested models relying on the $G^2$-statistic, which is approximately chi-square distributed \cite{Lewandowsky:2011}. 
...
The soft-max scaling factor $\alpha$ is only relevant in fPSIRSA, controlling how likely $S_1$ is to maximize utility when choosing utterances. 
...
$$ P(s_i \mid f_i) \prop \begin{cases}
\frac{1 + \gamma}, & \text{if}\ s_i \text{contains} f_i \\
\gamma, & \text{otherwise}
\end{cases},$$
boosting the choice of those objects $s_i$ that contain feature $f_i$. 
Note that $\gamma=0$ models a hard preference choice, that is, in that case the speaker is assumed to always choose one of the preferred objects. 
On the other hand, when $\gamma \rightarrow \infty$, the choice becomes uniform over all objects, effectively ignoring feature preferences. 
...
%objectPreferenceSoftPriors[[utt]] <- objectPreferenceSoftPriors[[utt]] / sum(objectPreferenceSoftPriors[[utt]])
 ...
 For example, in the trial shown in Figure \ref{exp1-trial} there are two objects that fit the utterance $u'=``red''$: a red striped cloud and a red dotted circle.
...
Finally, we allow for the possibility of noise in our human data introduced by participants not following instructions. The parameter $\beta$ models the possibility that listeners may choose objects that do not pass the semantic filter of the literal listener, that is, objects that do not match the received utterance $u'$. 

...
Moreover, some speakers can choose utterances that are expected to yield the most information about those preferences.
...
If a speaker and a listener interpret an ambiguous phrase differently, communication between them might fail. On rare occaisions, such communication failure is deadly. \citeA{pinker2015sense} alludes to the Charge of the Ligtht Brigade during the Crimear war as an example of a military disaster which was caused by vague orders. He further resports that poor wording on a warning light label that led to misinterpretation was also responsible for the nuclear meltdown at Three Mile Island in 1979. Finally, citing \citeA{cushing1994fatal}, Pinker describes a case of the deadliest plane crash in history that resulted from pilots and air traffic controllers arriving at different interpretations of the phrase 'at takeoff'.
...
As an alternative, we also test a model that relies on fewer layers of reasoning: an enhanced literal listener that now inlcudes prior preferences over states, and a speaker who ...

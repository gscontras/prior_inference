...
Figure~\ref{simple-full-individual} shows that the full model optimized at the individual level for the additional parameter $\alpha$ does not improve the fit compared to the simplified model (simplePSIRSA: $r^{2}=0.8631$; fullPSIRSA: $r^{2}=0.8614$). 
...
Note further that the individually-fitted parameters do not improve the correlation values much, if at all, when compared to the globally-fitted model, which changes when $\beta$ is optimized additionally. \gcs{I'm not sure I follow this last sentence.}

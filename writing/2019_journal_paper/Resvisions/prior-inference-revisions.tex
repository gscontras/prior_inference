...
While the classic RSA approach is aimed at inferring the meaning of an utterance by reasoning about the process that generated it, the inference process can also apply to aspects of the speaker's knowledge, or in other words, her priors. This type of inference is crucial for social interactions, since it helps the conversation partners build more accurate anticipatory models of each others' behavrior. The anticipatory nature of human cognition has been registered in a number of cognitive domains \cite{Butz:2016, belardinelli2016s, belardinelli2018mental,Friston:2015,Hayhoe:2003,lohmann2019hands} and is viewed by some theoreticians as a core property of the human mind \cite{Clark:2016}. 
...
This parameter allows the model to capture noise associated with adjusting sliders when no explicit evidence for the updating a particular type of preference was actually present in the scene.
...
The full and simple models with softness ($\gamma$) optimized globally provide nearly identically good fits to the data (Table \ref{global} \footnote{Here and throughout the paper we report adjusted $r^2$ values.} \footnote{All results were significant at the $p < 0.001$ level if not stated differently in the text.}).
...
\begin{tabular}{p{3.5cm}cccccc}
	...
	Softness & 0.8607 & 1,190 & 1181 & 0.2204 & & \\
	Softness \& Obedience & 0.9788 & 1,190 & 8823 & 0.2210 & 0.2693 & \\
	
	\hspace{1cm}\textit{Full model} & & & & & \\

	Softness & 0.8568 & 1,190 & 1144 & 0.2231 & & \\
	Softness \& Utility & 0.8607 & 1,190 & 1144 & 0.2205 &  & 0.1797\\
	Softness \& Obedience \& Utility & 0.9772 & 1,190 & 8170 & 0.2214 & 0.0030 & 0.2657\\
	\hline
\end{tabular}
\caption{Optimization summary (global)}
\label{global}
\end{table}
...
Moreover, the higher model flexibility of of the full model---controlled via parameter $\alpha$---does not yield any modeling improvement, implying that the performance in our task can be modeled as more shallow reasoning process within the simple model.
...
We then again averaged the individualized model prediction values and participants' slider values with respect to the particular ambiguity classes and calculated correlations between the data and the model.
	...
	\begin{table}[htb]
		...
		\begin{tabular}{p{4.5cm}ccc}
			...
			Softness & 0.8631 & 1,190 & 1205  \\
			Softness \& Obedience & 0.9919 & 1,190 & 23480  \\
			
			\hspace{1cm}\textit{Full model} & & & \\
			
			Softness \& Utility & 0.8627 & 1,190 & 1201 \\
			\hline
		\end{tabular}
		\caption{Optimization summary (individual), parameter values estimated for each subject separately.}
		\label{individual}
	\end{table}
...
The full model optimized at the individual level for the additional parameter $\alpha$ does not improve the fit compared to the simple model (Table \ref{individual}).
%(simple: $r^{2}=0.8631, F(1,190) = 1205$; full: $r^{2}=0.8627, F(1,190) = 1201$). 
...
As a result, the baseline model is much better in capturing variance in the data than one would expect without this insight (Table \ref{utterance_optimization}). The non-optimized model, surprisingly, captures very little variance in the human data. 
%($r^2=0.0595$, $F(1,82) = 6.253$, $p  < 0.05$). 
For  the non-optimized model we set the parameters to hard preference and full obedience ($\gamma=0$, $\beta=0$) and the information gain factor $\lambda$ to $1$, thus preferring to choose those utterances that are expected to yield high information gain. Figure~\ref{base-nonopt-x3} compares this performance to the non-optimized simple RSA model and a baseline model that relies on a simple heuristic of distributing the probability mass equally between all utterances that are available in a scene.
...
When optimizing all of the simple RSA model parameters, the model accounts for more variance than the uniform base model 
...
The parameters indicate that the preference strength is rather high, obedience is not as strong, while the information gain intention is present (Table \ref{utterance_optimization}). 
...
Previous work in the RSA framework has focused on jointly inferring meaning of utterances and the state of the world based on the speaker's choice of utterances \cite{degen2015wonky, hawkins2015you} and the iterative vocabulary learning tasks that involve the inference about the priors of the speaker  \cite{woensdregt2016modelling, blokpoel2019pragmatic}.

...
Having fit the different parameter values, we then compared various optimized models with our un-fit, base model (\gcs{what were the default parameter values in the base model?}). The base model had $\alpha$ set to 1, while $\beta$ and $\gamma$ were set to 0. To evaluate the performance of the models, we relied on KL divergence values to assess  goodness of fit. These values were calcuated by taking into account the observed feature preferences $f$ and the difference of $f$ and model-predicted feature preferences $f'$:  
...
As with Experiment 1, here we collapsed across specific feature values to classify our stimuli into \gcs{how many?} 14 conditions. For each condition, we computed the average utterance preference values \gcs{across participants?}. Means were later normalized. \gcs{what was done in the normalization step?}

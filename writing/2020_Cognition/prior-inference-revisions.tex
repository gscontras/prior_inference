...
% ($\lambda=0.0069$; $r^2=0.8013$, $F(1,82) = 335.8$, $p<0.001$). 
%% ($r^2=0.80$; $G^2 = 13.6912,  p<0.01$).
...
%  yielded comparable values (softness optimized to $\gamma=0.2778$: $r^2=0.8134$, $F(1,82) = 362.9$, $p<0.001$; obedience optimized to $\beta=0.1421$: $r^2=0.8048$, $F(1,82) = 343.1$, $p<0.001$)
The results show that the optimized simple models act quite similar to the uniform model.
% Moreover, the analysis of the obtained parameter values (Table~\ref{utterance_optimization}) suggests that the parameter of primary interest $\lambda$ has a rather small value of $0.3663$, indicating only moderate attention to information gain on the part of participants. 
...
%When optimizing for softness or obedience, the non-zero values tend to blend the utterance choices in a somewhat similar manner (cf. Figure~\ref{barplot_x3}).
%Thus, optimizing either one of the three parameters pushes towards a uniform utterance choice tendency. 
%This result speaks against a shared reasoning strategy by all participants. 
% This result suggests that  or it could come as a consequence of averaging over a heterogeneous group, suggesting an analysis of between-subject differences.

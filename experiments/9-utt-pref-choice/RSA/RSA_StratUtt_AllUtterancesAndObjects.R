...
# 
getMapUttToObjToPref <- function(relevantObjects, allObjects, allUtterancePref, mapUttToObjProbs, allUtterancesNew1){
  mapUttToObjToPref <- mapUttToObjProbs
  for (obj in rep(1:length(relevantObjects))){
    object <- relevantObjects[obj]
    objectSpecific <- allObjects[object,]
    for (featureValue in rep(1:length(allUtterancesNew1))){
      pref <- allUtterancePref[which(allUtterancePref[,2] == objectSpecific[featureValue]), 3]
      index <- which(relevantUtterances == objectSpecific[featureValue])
      #cat(pref)
      #cat(index)
      cat(objectSpecific[featureValue])
      #cat(objectSpecific)

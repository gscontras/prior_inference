...
###################################################################################
parOptType <-4 ######## 1,2,3 params OR 1 and not obey.1 (= parOptType=4) number of parameters to be optimized. #####################
###################################################################################
...
if(parOptType==1) {
  ...
  paramsWorkers12 <- matrix(0,length(unique(workerIDs)), 45)
  ...
  if(length(idICases)>0) {
    ...
    for(workerTrialIndex in c(1:length(idICases))) {
      ...
      if(parOptType==1) {
        ...
        optResAll3 <- optim(c(.2, .2, .2), RSAModelLL3, method="L-BFGS-B", gr=NULL, dataWorker,
                            lower = c(0,0,0.01), upper = c(1e+10,1e+10,100))
        llWorkers12[workerIndex,workerTrialIndex] <- optResAll3$value
        paramsWorkers12[workerIndex,(workerTrialIndex-1)*3+1] <- optResAll3$par[1]
        paramsWorkers12[workerIndex,(workerTrialIndex-1)*3+2] <- optResAll3$par[2]
        paramsWorkers12[workerIndex,(workerTrialIndex-1)*3+3] <- optResAll3$par[3]
      }else if(parOptType==4) {
        optRes1 <- optimize(RSAModelLL1_1_notObey.1, c(0,1e+10), dataWorker)
        llWorkers12[workerIndex,workerTrialIndex] <- optRes1$objective
        paramsWorkers12[workerIndex,workerTrialIndex] <- optRes1$minimum
      }
      ...
      if(parOptType==1) {
        klDivCrossValValues[workerIndex, workerTrialIndex] <-
          RSAModelKLDiv3paramsOnlyAvailableFeatureValuesConsidered(dataTrialWorker, abs(optRes1$minimum), 0, 1)
      }else if(parOptType==2) {
        klDivCrossValValues[workerIndex, workerTrialIndex] <- 
          RSAModelKLDiv3paramsOnlyAvailableFeatureValuesConsidered(dataTrialWorker, optRes2n3$par[1], optRes2n3$par[2], 1)
      }else if(parOptType==3) {
        klDivCrossValValues[workerIndex, workerTrialIndex] <- 
          RSAModelKLDiv3paramsOnlyAvailableFeatureValuesConsidered(dataTrialWorker, optResAll3$par[1], optResAll3$par[2], optResAll3$par[3])
      }else if(parOptType==4) {
        klDivCrossValValues[workerIndex, workerTrialIndex] <-
          RSAModelKLDiv3paramsOnlyAvailableFeatureValuesConsidered(dataTrialWorker, abs(optRes1$minimum), .1, 1)
      }
...
## writing out tables
if(parOptType==1) {
  write.csv(llWorkers12, "X2_Data/x2CrossVal_KLDivOpt_1parOpt_2019_0430.csv")
  write.csv(paramsWorkers12, "X2_Data/x2CrossVal_Params_1parOpt_2019_0430.csv")
  write.csv(klDivCrossValValues, "X2_Data/x2CrossVal_KLDivTrial_1parOpt_2019_0430.csv")
}else if(parOptType==2) {
  write.csv(llWorkers12, "X2_Data/x2CrossVal_KLDivOpt_12parOpt_2019_0430.csv")
  write.csv(paramsWorkers12, "X2_Data/x2CrossVal_Params_12parOpt_2019_0430.csv")
  write.csv(klDivCrossValValues, "X2_Data/x2CrossVal_KLDivTrial_12parOpt_2019_0430.csv")
}else if(parOptType==3) {
  write.csv(llWorkers12, "X2_Data/x2CrossVal_KLDivOpt_123parOpt_2019_0430.csv")
  write.csv(paramsWorkers12, "X2_Data/x2CrossVal_Params_123parOpt_2019_0430.csv")
  write.csv(klDivCrossValValues, "X2_Data/x2CrossVal_KLDivTrial_123parOpt_2019_0430.csv")
}else if(parOptType==4) {
  write.csv(llWorkers12, "X2_Data/x2CrossVal_KLDivOpt_notObey.1_2019_0430.csv")
  write.csv(paramsWorkers12, "X2_Data/x2CrossVal_Params_notObey.1_2019_0430.csv")
  write.csv(klDivCrossValValues, "X2_Data/x2CrossVal_KLDivTrial_notObey.1_2019_0430.csv")
}
